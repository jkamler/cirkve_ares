	...
	function importRUIAN() {
		try {
			...
			foreach ($RUIANFiles as $fileName) {
				...
				//cut first line - first line is header of table and do not contain data
				...
				foreach ($fileContent as $fileLine) {
					...
					echo "count " . count($arr_sql);
					for($i = 0; $i < count($arr_sql) - 1; $i++)  {
						if ($i > 11 && $i < 15 ) { //items without "'" --data type numeric  
							$sql .= $arr_sql[$i] . ", ";
							echo "I: " . $i;
							continue;
						}
					...
					$sql .= "'" . $arr_sql[count($arr_sql)-1] . "')"; //last item
					if (!mysqli_query($conn, $sql)) {
